<template>
  <div id="app">
    <a-row>
      ...
      <a-col :span="12">
        <div>
           <a-input-search placeholder="input search text" v-model="searchStr" size="large" @input="searchInput" @search="search">
             <a-button slot="enterButton">搜索</a-button>
            </a-input-search>
            <ul>
              <li v-for="d in searched" :key="d">
                <div class="wrap-list" @mouseenter="isActive = d" @mouseleave="isActive = null" v-bind:class="{active: isActive == d}">
                  <div>{{d}}</div>
                  <div class="icon-list">
                    <a title="访问github" :href="gitHubAddress(d)" target="_blank">
                    ...
                    <a title="访问npm" :href="npmAddress(d)" target="_blank">
...
export default {
  ...
  methods: {
      getTypesPathShaValue: function() {
        ...
        const sha = sessionStorage.getItem(pathName);
        if (sha) {
          self.shaValue = sha;
           self.initData();
          return;
        ...
        this.$http.get(url).then(resp => {
          ...
          if (resp.ok && 200 === resp.status) {
            ...
            if (info) {
              sessionStorage.setItem(pathName, info.sha); // 将sha值，放入缓存
      ...
      gitHubAddress: function(name) {
        return `https://github.com/DefinitelyTyped/DefinitelyTyped/tree/master/types/${name}`;
      },
      npmAddress: function(name) {
        return `https://www.npmjs.com/package/@types/${name}`;
      }
  },
  mounted() {
    console.log('mounted');
